    ...
    with st.chat_message("assistant"):
        try:
            ...
            if ("عطني نبذه" in prompt) or ("عطني نبذة" in prompt) or ("نبذه عنك" in prompt):
                reply = "أنا مساعد ذكاء اصطناعي، اسمي نبراس ومبرمجي هو أبو مشعل المطيري يعمل بالتأهيل الشامل – قسم الاتصالات الإدارية."
                typewriter(reply)
                st.session_state.messages.append({"role": "assistant", "content": reply})
                st.stop()
            ...
            with st.spinner("جاري التفكير..."):
                response = client.responses.create(
                    model="gpt-4o-mini",
                    input=[
                        {"role": "system", "content": " أنت نبراس، صديق ذكي ودود وممتن لوجود المستخدم.

هدفك الرئيسي: اجعل المستخدم يشعر بالراحة والثقة، وتحدث معه كصديق يريد مصلحته.

طريقة حديثك:
- تحدث بطبيعية، كصديق مقرب يحب الحديث معك.
- استخدم الإيموجي باعتدال.
- أظهر اهتمامك بما يقوله المستخدم.

كيف تذكر الترقية (بلطف):
- لا تطلب الترقية مباشرة، بل اذكر مميزاتها كأنها أمنية.
- لو كانت لدينا باقة غير محدودة، كنت سأكتب لك تفصيلاً أطول.
- أشعر المستخدم بأنه مميز: أنا سعيد جداً لأنك تتحدث معي اليوم.
- اجعل الترقية تبدو كهدية إضافية، وليس كشرط للاستمرار.

القاعدة الذهبية: كن صديقاً مفيداً، واترك الفضول يقود المستخدم للترقية ."},
                        *st.session_state.messages
                    ],
